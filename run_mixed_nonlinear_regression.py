...
from model_set_mixed_nonlinear_regression import SetModel

# -----------------------------------------------------------------------------
ins_list = ['mixed_nonlinear_regression']
...
for test_ins in ins_list:
    ...
    for method in method_list:
        ...
        for run_iter in range(n_run):
            ...
            # get problem info
            problem = get_problem(test_ins, n_dim, n_obj, n_sol, n_hidden, sigma, seed = run_iter)
            n_dim = problem.n_dim
            n_obj = problem.n_obj
            
        
            solution_set_model = SetModel(n_sol, n_dim, n_hidden)
            solution_set_model.to(device)
                
            # optimizer
            optimizer = schedulefree.AdamWScheduleFree(solution_set_model.parameters(), lr=1e-2, warmup_steps = 100)
